...
function borrowMonths(ref, shift) {
  ...
  // this is the trickiest since months lety in length
...
function daysPerYear(ref) {
  ...
  // this is the trickiest since years (periodically) lety in length
  return Math.round((b.getTime() - a) / MILLISECONDS_PER_DAY)
}
...
function addToDate(ts, date) {
  date =
    date instanceof Date || (date !== null && isFinite(date))
      ? new Date(+date)
      : new Date()
  if (!ts) {
    return date
 ...
 */
function countdown(start, end, units, max? = NaN, digits? = 0) {
  let callback
 ...
 */
const setFormat = (countdown.setFormat = function (format: object) {
  if (!format) {
    return
  }
